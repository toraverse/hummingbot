...
class BigoneAPIOrderBookDataSourceUnitTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.connector = BigoneExchange(
            ...
            domain=self.domain)
        self.data_source = BigoneAPIOrderBookDataSource(trading_pairs=[self.trading_pair],
    ...
    def test_get_new_order_book_successful(self, mock_api):
        url = web_utils.public_rest_url(path_url=CONSTANTS.SNAPSHOT_PATH_URL.format(self.ex_trading_pair), domain=self.domain)
        regex_url = f"{url}?limit=200"
    ...
    def test_listen_for_subscriptions_subscribes_to_trades_and_order_diffs(self, ws_connect_mock):
        ...
        result_subscribe_diffs = json.dumps({
            "code": None,
            "id": 1
        }).encode('utf-8')
        ...
        self.mocking_assistant.add_websocket_aiohttp_message(
            websocket_mock=ws_connect_mock.return_value,
            message=result_subscribe_diffs)
        ...
        expected_trade_subscription = {
            "requestId": "1",
            "subscribeMarketTradesRequest": {"market": self.ex_trading_pair, "limit": "20"}
        }
        self.assertEqual(expected_trade_subscription, sent_subscription_messages[0])
        expected_diff_subscription = {
            "requestId": "1",
            "subscribeMarketDepthRequest": {"market": self.ex_trading_pair}
        }
    ...
    def test_listen_for_order_book_diffs_successful(self):
        ...
        msg: OrderBookMessage = self.async_run_with_timeout(msg_queue.get())
        diff = json.loads(diff_event)

        self.assertEqual(int(diff["depthUpdate"]["changeId"]), msg.update_id)
    ...
    @aioresponses()
    @patch("hummingbot.connector.exchange.bigone.bigone_api_order_book_data_source"
           ".BigoneAPIOrderBookDataSource._sleep")
