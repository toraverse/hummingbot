...
class BtseAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _subscribe_channels(self, ws: WSAssistant):
        ...
        try:
            ...
            self.logger().info("Subscribed to public order book and diff channel.")
        except asyncio.CancelledError:
            raise
        except Exception:
            self.logger().error(
                "Unexpected error occurred subscribing to order book and diff streams...",
                exc_info=True
            )
            raise
    ...
    async def _connected_websocket_assistant(self) -> WSAssistant:
        ...
        ws_url = f"{CONSTANTS.WSS_ORDER_BOOK_URL.format(self._domain)}"
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot["bids"] = [[bid["price"], bid["size"]] for bid in snapshot["buyQuote"]]
        snapshot["asks"] = [[bid["price"], bid["size"]] for bid in snapshot["sellQuote"]]

        del snapshot["buyQuote"]
        del snapshot["sellQuote"]
    ...
    async def _parse_order_book_snapshot_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        if "data" in raw_message:
            trading_pair = await self._connector.trading_pair_associated_to_exchange_symbol(symbol=raw_message["data"]["symbol"])
            order_book_message: OrderBookMessage = BtseOrderBook.snapshot_message_from_exchange(
                raw_message["data"], time.time(), {"trading_pair": trading_pair})
            message_queue.put_nowait(order_book_message)
    ...
    def _channel_originating_message(self, event_message: Dict[str, Any]) -> str:
        ...
        if "data" in event_message:
            event_type = event_message["data"].get("type")
            channel = (self._snapshot_messages_queue_key if event_type == CONSTANTS.SNAPSHOT_EVENT_TYPE
