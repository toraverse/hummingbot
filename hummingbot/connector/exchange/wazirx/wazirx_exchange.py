...
class WazirxExchange(ExchangePyBase):
    ...
    def __init__(self,
        ...
        self._last_trades_poll_wazirx_timestamp = 1.0

        self._max_trade_id_by_symbol: Dict[str, int] = dict()
        super().__init__(client_config_map)

    @staticmethod
    def wazirx_order_type(order_type: OrderType) -> str:
        return CONSTANTS.ORDER_TYPE_MAP[order_type]

    @staticmethod
    def to_hb_order_type(wazirx_type: str) -> OrderType:
        return OrderType[wazirx_type]
    ...
    async def _place_order(self,
        ...
        type_str = CONSTANTS.ORDER_TYPE_MAP[order_type]
        ...
        try:
            ...
            transact_time = order_result["updatedTime"] * 1e-3
        except IOError as e:
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        cancel_result = await self._api_delete(
            ...
            params=api_params,
            is_auth_required=True)
        if cancel_result.get("status") == "wait":
            return True
        return False
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        retval = []
        for rule in filter(wazirx_utils.is_exchange_information_valid, trading_pair_rules):
            try:
                ...
                tick_size = price_filter.get("minPrice")
                step_size = Decimal(price_filter.get("tickSize"))
    ...
    async def _user_stream_event_listener(self):
        ...
        async for event_message in self._iter_user_event_queue():
            try:
                channels: List[str] = event_message.get("data", {}).get("streams", [])
                channel: str = event_message.get("stream", None)
                events: Dict[str, Any] = event_message.get("data", {}).get("event", {})
                results: Dict[str, Any] = event_message.get("data", {})
                if "subscribed" in events and channel not in channels:
                    self.logger().error(
    ...
    def _process_balance_message_ws(self, account):
        ...
        for balance_entry in balances:
            ...
            free_balance = Decimal(balance_entry["b"])
            total_balance = Decimal(balance_entry["b"]) + Decimal(balance_entry["l"])
    ...
    def _create_trade_update_with_order_fill_data(
        ...
        fee = TradeFeeBase.new_spot_fee(
            ...
            percent_token=order_fill["U"],
            flat_fees=[TokenAmount(amount=Decimal(order_fill["f"]), token=order_fill["U"])]
        )
        trade_update = TradeUpdate(
            trade_id=str(order_fill["t"]),
            client_order_id=order.client_order_id,
            exchange_order_id=str(order_fill["o"]),
            trading_pair=order.trading_pair,
            fee=fee,
            fill_base_amount=Decimal(order_fill["q"]),
            fill_quote_amount=Decimal(order_fill["q"]) * Decimal(order_fill["p"]),
            fill_price=Decimal(order_fill["p"]),
            fill_timestamp=order_fill["E"] * 1e-3,
        )
        return trade_update
    ...
    def _create_order_update_with_order_status_data(self, order_status: Dict[str, Any], order: InFlightOrder):
        client_order_id = str(order_status["data"].get("c", ""))
        order_update = OrderUpdate(
            trading_pair=order.trading_pair,
            update_timestamp=order_status["data"]["E"] * 1e-3,
            new_state=CONSTANTS.ORDER_STATE[order_status["data"]["X"]],
            client_order_id=client_order_id,
            exchange_order_id=str(order_status["data"]["i"]),
        )
        return order_update

    def _process_order_message(self, raw_msg: Dict[str, Any]):
        # order_msg = raw_msg.get("data", {})
        client_order_id = raw_msg["data"].get("c", "")
    ...
    #     in case Wazirx's user stream events are not working.
    #     NOTE: It is not required to copy this functionality in other connectors.
    #     This is separated from _update_order_status which only updates the order status without producing filled
    #     events, since Wazirx's get order endpoint does not return trade IDs.
    ...
    #         query_time = int(self._last_trades_poll_wazirx_timestamp * 1e3)
    #         self._last_trades_poll_wazirx_timestamp = self._time_synchronizer.time()
    ...
    #                             order_type=OrderType.LIMIT if trade["isBuyerMaker"] else OrderType.LIMIT_MAKER,
            ...
            for _ in range(2):
                ...
                for trade_data in result:
                    if str(trade_data["orderId"]) in orders_to_process:
                        ...
                        fee_token = trade_data["feeCurrency"].upper()  # typo in the json by the exchange
                        fee = TradeFeeBase.new_spot_fee(
                            fee_schema=self.trade_fee_schema(),
                            trade_type=order.trade_type,
                            percent_token=fee_token,
                            flat_fees=[TokenAmount(amount=Decimal(trade_data["fee"]), token=fee_token)],
                        )
                        trade_update = TradeUpdate(
                ...
                if len(result) > 0:
                    self._max_trade_id_by_symbol[symbol] = max(int(t["id"]) for t in result)
                if len(result) < 1000:
                    break
    ...
    def _initialize_trading_pair_symbols_from_exchange_info(self, exchange_info: Dict[str, Any]):
        mapping = bidict()
        for symbol_data in filter(wazirx_utils.is_exchange_information_valid, exchange_info["symbols"]):
